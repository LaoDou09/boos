...
class Money {
    ...
    public function gt($num1,$num2){
        if($this->sub($num1,$num2) > 0)
            return true;
        else{
            return false;
        }
    }
    public function gte($num1,$num2){
        if($this->sub($num1,$num2) >= 0)
            return true;
        else
            return false;
    ...
    public function lt($num1,$num2){
        if($this->sub($num1,$num2) < 0)
            return true;
        else
            return false;
    }
    public function lte($num1,$num2){
        if($this->sub($num1,$num2) <= 0)
            return true;
        else
            return false;
    ...
    public function levelGt($level1,$level2){
        return $this->gt(substr($level1,1),substr($level2,1));
    }


    //格式化
    public function format($num,$f=0)
    {
        $f=0;
        $tmp =$num *100;
        $ret = floor($tmp."") / 100;
    ...
    //自动判断
    public  function yuan2Fen($num)
    {
        if(strpos($num,'.') != false){
            //有小数点
            return $this->getYuan2Fen($num);
        }else{
            return $num;
        }
    }

    public function fen2Yuan($fen)
    {
        $yuan = $this->div($fen,100);
        return $this->format($yuan);
    }
